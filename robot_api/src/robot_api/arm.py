...
class Arm(object):
    ...
    def move_to_pose(self,
        ...
        # Send the goal and wait for a specified time
        self._move_group_client.send_goal(goal)
        if self._move_group_client.wait_for_result(rospy.Duration(10)):  # Wait for up to 10 seconds
            ...
            if result and result.error_code.val == MoveItErrorCodes.SUCCESS:
                ...
                # If not successful, return an error string
                return self.moveit_error_string(result.error_code.val if result else MoveItErrorCodes.TIMED_OUT)
        else:
            # If the result wasn't received in time, consider it a timeout error
            rospy.logerr("MoveIt action client timed out while waiting for a response.")
            return 'TIMED_OUT'
